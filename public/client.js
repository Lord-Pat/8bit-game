...
function connect() {
  const configuredUrl = typeof window.SOCKET_URL === 'string' ? window.SOCKET_URL.trim() : '';
  const backendUrl = configuredUrl || location.origin;
  const socketUrl = new URL(backendUrl, location.href);
  socketUrl.protocol = socketUrl.protocol === 'https:' ? 'wss:' : 'ws:';
  const hostKey = new URLSearchParams(location.search).get('host');
  if (hostKey) socketUrl.searchParams.set('host', hostKey);
